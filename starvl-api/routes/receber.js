    ...
    const recebidosResult = await query(
      `SELECT
       ...
       WHERE recj.recjempresa = $1
         AND DATE(recj.recjfechamento) >= $2
         AND DATE(recj.recjfechamento) <= $3`,
      [empresa, mesInicio, d]
    );
...
router.get('/contas', async (req, res) => {
  ...
  const limit     = Math.min(500, Math.max(5, parseInt(req.query.limit) || 10));
  ...
  try {
    ...
    const statusSQL = {
      a_vencer:   `AND rece.recevencimento > '${d}'`,
      vence_hoje: `AND DATE(rece.recevencimento) = '${d}'::date`,
      atrasado:   `AND rece.recevencimento < '${d}'`,
      recebido:   `AND 1 = 0`,
    }[status] || '';
    ...
    const dataSQL = `
      SELECT
        ...
        END AS status,
        CASE
          WHEN rece.recevencimento < '${d}' THEN ('${d}'::date - DATE(rece.recevencimento))::int
          ELSE 0
        END AS dias_atraso,
        NULL AS data_recebimento
      ${baseFrom}
      ORDER BY
        CASE WHEN rece.recevencimento < '${d}' THEN 0 ELSE 1 END,
...
router.get('/analiticos', async (req, res) => {
  ...
  try {
    const statusRes = await query(
      `SELECT
         (SELECT COALESCE(SUM(recj.recjvalor), 0) FROM recj WHERE recj.recjempresa = $1) AS recebido,
         COALESCE(SUM(CASE WHEN DATE(rece.recevencimento) = $2::date THEN rece.recevalor END), 0) AS vence_hoje,
         COALESCE(SUM(CASE WHEN rece.recevencimento < $2 THEN rece.recevalor END), 0)  AS atrasado,
    ...
    const indicesRes = await query(
      `SELECT
         ...
         (SELECT COALESCE(AVG((DATE(recj.recjfechamento) - DATE(recj.recjvencimento))::int), 0)
            FROM recj WHERE recj.recjempresa = $1) AS prazo_medio,
         (SELECT COUNT(CASE WHEN recj.recjfechamento < recj.recjvencimento THEN 1 END)::float /
